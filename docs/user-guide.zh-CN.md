...
| 工具名 | 说明 |
| --- | --- |
| `web_search` | 联网搜索公开网页。`provider=auto` 时优先使用 Tavily；未配置 `TAVILY_API_KEY` 时使用 DuckDuckGo HTML 搜索，DuckDuckGo 不可用时回退到 Bing。 |
...
`TAVILY_API_KEY` 可以省略；省略后 `web_search` 自动使用 DuckDuckGo HTML 搜索，DuckDuckGo 不可用时回退到 Bing。
...
未设置 `TAVILY_API_KEY` 时，`web_search` 会自动使用 DuckDuckGo HTML 搜索；如果当前网络无法连接 DuckDuckGo，会回退到 Bing。需要使用 Tavily 时再设置：

```sh
export TAVILY_API_KEY='your-tavily-key'
```

`provider=auto` 优先使用 Tavily；未配置 key 时使用 DuckDuckGo，失败后回退到 Bing。显式传入 `provider=tavily` 但未配置 key 时，也会走同一套公共搜索回退链。
