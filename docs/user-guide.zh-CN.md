...
| 工具名 | 说明 |
| --- | --- |
| `web_search` | 联网搜索公开网页，只保留 Bing 和 Tavily。`provider=auto` 时优先使用 Tavily；未配置 `TAVILY_API_KEY` 时使用 Bing；Bing 搜索最多返回 50 条并会自动翻页。 |
...
| `TAVILY_API_KEY` | 空 | Tavily API Key。为空时 `provider=auto` 使用 Bing；显式 `provider=tavily` 需要设置。 |
...
`TAVILY_API_KEY` 可以省略；省略后 `provider=auto` 使用 Bing。显式 `provider=tavily` 需要设置该变量。
...
`max_results` 实际限制在 1 到 50；使用 Bing 搜索时，请求超过第一页结果会自动继续请求下一页并按 URL 去重。
...
未设置 `TAVILY_API_KEY` 时，`provider=auto` 会使用 Bing；Bing 请求超过第一页结果时会自动翻页。需要使用 Tavily 时再设置：

```sh
export TAVILY_API_KEY='your-tavily-key'
```

`provider=auto` 优先使用 Tavily；未配置 key 时使用 Bing。显式传入 `provider=tavily` 但未配置 key 时会报错；显式传入 `provider=bing` 始终使用 Bing。
