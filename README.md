...
- `web_search` 联网搜索：只保留 Bing 和 Tavily；`provider=auto` 有 `TAVILY_API_KEY` 时优先使用 Tavily，否则使用 Bing；也可显式选择 `provider=bing` 或 `provider=tavily`
...
| 工具名 | 说明 |
| --- | --- |
| `web_search` | 联网搜索公开网页，只保留 Bing 和 Tavily；`provider=auto` 优先使用 Tavily，未配置 Key 时使用 Bing；可显式选择 `provider=bing` 或 `provider=tavily`，Bing 搜索最多返回 50 条并会自动翻页。 |
